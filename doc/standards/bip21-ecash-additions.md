...
- Simple send (single recipient): use `token_decimalized_qty` with the leading URI address.
- Send-to-many: include additional `addr` params, each immediately followed by a `token_decimalized_qty` for that `addr`.
...
#### Bip-21 URI for an SLP or ALP Send-to-many tx
...
In this transaction, 1.23 tokens are sent to the leading URI address, 2 tokens are sent to `ecash:qz2708636snqhsxu8wnlka78h6fdp77ar59jrf5035`, and 3.5 tokens are sent to `ecash:qp89xgjhcqdnzzemts0aj378nfe2mhu9yvxj9nhgg6`. Each token output contains 546 satoshis.
