...
//! PCP mapping nonce. Arbitrary data chosen by the client to identify a
//! mapping.
...
enum class MappingError {
    ...
    NETWORK_ERROR,
    //! Any kind of protocol-level error, except unsupported version
    //! or no resources.
    PROTOCOL_ERROR,
...
struct MappingResult {
    MappingResult(uint8_t version, const CService &internal_in,
                  const CService &external_in, uint32_t lifetime_in)
        : version(version), internal(internal_in), external(external_in),
...
//!
//! * gateway: Destination address for PCP requests (usually the default
//!   gateway).
//! * port: Internal port, and desired external port.
//! * lifetime: Requested lifetime in seconds for mapping. The server may assign
//!   as shorter or longer lifetime. A lifetime of 0 deletes the mapping.
//! * num_tries: Number of tries in case of no response.
//!
//! Returns the external_ip:external_port of the mapping if successful,
...
//! Try to open a port using RFC 6887 Port Control Protocol (PCP). Handles IPv4
//! and IPv6.
//!
//! * nonce: Mapping cookie. Keep this the same over renewals.
//! * gateway: Destination address for PCP requests (usually the default
//!   gateway).
//! * bind: Specific local bind address for IPv6 pinholing. Set this as
//!   INADDR_ANY for IPv4.
//! * port: Internal port, and desired external port.
//! * lifetime: Requested lifetime in seconds for mapping. The server may assign
//!   as shorter or longer lifetime. A lifetime of 0 deletes the mapping.
